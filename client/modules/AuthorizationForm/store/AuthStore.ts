...
export const useAuthStore = defineStore("auth", {
  ...
  getters: {
    ...
    isAdmin: (state: AuthState) => {
      if (state._user) {
        return (
          state._isAuth && DASHBOARD_ADMIN_ROLES.includes(state._user.role)
        );
      }
      return false;
    },
    isAdminOrRedactor: (state: AuthState) => {
      if (state._user) {
        return (
          state._isAuth && DASHBOARD_ACCESS_ROLES.includes(state._user.role)
        );
      }
      return false;
    },
    ...
    async checkAuth() {
      try {
        ...
        const response = await axios.get(
          `http://localhost:5009/api/user/refresh`,
